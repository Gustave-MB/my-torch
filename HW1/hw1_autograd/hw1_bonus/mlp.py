...
class MLP(object):
    ...
    def forward(self, x):
        ...
        for i in range(self.nlayers):
            if i < self.nlayers - 1:
                ...
                out = self.activations[i](np.dot(self.linear_layers[i].W, out) + self.linear_layers[i].b)
            else:
                # No activation for the output layer
                out = np.dot(self.linear_layers[i].W, out) + self.linear_layers[i].b
    ...
    def step(self):
        ...
        for layer in self.linear_layers:
            # Update the weights and biases using an optimizer
            # For example, using SGD with learning rate self.lr:
