...
class MSELoss:
    ...
    def forward(self, A, Y):
        ...
        self.ln = np.ones((self.N,1))
        self.lc = np.ones((self.C,1))
        se = (A-Y) * (A-Y)  # TODO
        sse = self.ln.T @ se @ self.lc # TODO
...
class CrossEntropyLoss:
    ...
    def forward(self, A, Y):
        ...
        self.softmax = np.exp(self.A - np.max(self.A, axis=1, keepdims=True)) / np.sum(np.exp(self.A - np.max(self.A, axis=1, keepdims=True)), axis=1, keepdims=True)  # TODO
